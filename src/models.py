...
from modules.encoders import LanguageEmbeddingLayer, SeqEncoder
...
class MULTModel(nn.Module):
    ...
    def forward(self, sentences, video, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask):
        ...
        if self.hp.use_bert:
            enc_word = self.embedding(sentences, lengths, bert_sent, bert_sent_type,
                                      bert_sent_mask)  # (batch_size, seq_len, emb_size), (batch_size, emb_size)
            video, acoustic, lengths = _pad_seq(video, acoustic, lengths)
        else:
            enc_word = self.embedding(sentences, lengths, bert_sent, bert_sent_type,
                                      bert_sent_mask)  # (seq_len, batch_size, emb_size)
        ...
        last_a2l, last_l2a, disc_pred_la, disc_true_la = self.modal_interaction['la'](seq_l, seq_a, h_l, h_a, lengths,
                                                                                      mask)
        last_v2l, last_l2v, disc_pred_lv, disc_true_lv = self.modal_interaction['lv'](seq_l, seq_v, h_l, h_v, lengths,
                                                                                      mask)
